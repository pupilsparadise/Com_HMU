...
static OtdGsmApp_Status_ten OtdGsmApp_GsmRxProcess(OtdGsmApp_SubState_ten rx_state)
{
	...
	if(RxIndex > 0)
	{
		...
		switch(rx_state)
		{
			case GSM_AT_RST:
					#if GSM_DEBUG
					OtdUart_DebugSend("GSM_DEBUG >> GSM Rx response====Processing AT REST command=====\n");
					#endif	
					
					expected_reply = "Call Ready";//update expected reply
			...
			case GSM_CFUN:
					#if GSM_DEBUG
					OtdUart_DebugSend("GSM_DEBUG >> GSM Rx response====Processing GSM_CFUN command=====\n");
					#endif	
			...
			case GSM_COPS_CHECK:
					...
					expected_reply = "OK";//"Vi";//update expected reply //TODO: store the opearator name
	...
	switch(GsmSubState_st.Current_en)
	{
		case GSM_AT_RST:
				if(!GsmSubState_st.CmdSendFlag)
				{
					...
					OtdGsmApp_SendATCommand("AT+RESET\r\n");
					GsmSubState_st.TimeOut = RX_TIMEOUT;
					#if GSM_DEBUG
					OtdUart_DebugSend("GSM_DEBUG >> GSM Sub-StateMachine====AT+RESET=====send\n");
					#endif	
		...
		case GSM_CFUN:
				...
				{
					OtdGsmApp_ClearRxBuffer();//Clear Rx Buffer 
					OtdGsmApp_SendATCommand("AT+CFUN=1,1\r\n");
					GsmSubState_st.TimeOut = RX_TIMEOUT;
					#if GSM_DEBUG
					OtdUart_DebugSend("GSM_DEBUG >> GSM Sub-StateMachine====AT+CFUN=====send\n");
					#endif	
...
void OtdGsmApp_GsmStateInit(void)
{
	...
	GsmSubState_st.Current_en = GSM_CFUN;//GSM_AT_RST;
	GsmSubState_st.Previous_en = GsmSubState_st.Current_en;
