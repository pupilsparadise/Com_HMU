...
extern unsigned char CanRecBuffer[10];

extern unsigned char RxCanDataFlg;
...
void mcp2517_recv_interrupt()
{
    ...
    uint8_t data[64];
     ...
     mcp2518fd_TransmitChannelEventGet(APP_RX_FIFO, &rxFlags);
 
     if (rxFlags & CAN_RX_FIFO_NOT_EMPTY_EVENT) 
     {
        mcp2518fd_ReceiveMessageGet( APP_RX_FIFO, &rxObj, rxd, MAX_DATA_BYTES);
        DRV_CANFDSPI_ModuleEventClear(CAN_RX_EVENT);
	
        dlc = DRV_CANFDSPI_DlcToDataBytes( (CAN_DLC) rxObj.bF.ctrl.DLC);
	
        //sprintf(data,"Received ID 0x%x  with %d bytes \r\n", rxObj.bF.id.SID, dlc);
	//OtdUart_DebugSend_CAN(data);
	
        for(i=0; i<dlc; i++){
		CanRecBuffer[i] = rxd[i];
		//R_UART1_Send(CanRecBuffer[i], 1);
		//sprintf(data,"%02x ", rxd[i]);
		//OtdUart_DebugSend_CAN(data);
	}
	RxCanDataFlg = 1; 
	
//	RxCanDevID = (CanRecBuffer[0] << 8) | CanRecBuffer[1];
	//sprintf(data,"Received Data %d",rcv_count,CanRecBuffer);
	//sprintf(data, "rx%d: %s\r\n", RxCanDevID, CanRecBuffer);
        //OtdUart_DebugSend_CAN("CAN RX\n");
